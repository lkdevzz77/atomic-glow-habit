-- Corrigir security warnings: adicionar search_path seguro nas funções

-- Recriar função get_habit_completion_xp com search_path seguro
CREATE OR REPLACE FUNCTION get_habit_completion_xp(p_user_id UUID, p_habit_id BIGINT, p_date DATE)
RETURNS TABLE(total_xp INT, reasons TEXT[]) 
LANGUAGE plpgsql
SECURITY DEFINER
